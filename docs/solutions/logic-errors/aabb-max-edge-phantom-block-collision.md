...
- **Don't hand-roll an AABB voxel scan — use `@engine/physics`.** `maxBlock` for max edges, `firstBlockingLayer` to find what stops a move. Two independent defects came from hand-rolled loops: the inclusive max edge, and scanning the moving axis in the wrong order. Grep before adding a loop: `grep -rn "<= Math.floor(max" src/` should return nothing.
- **Scan the moving axis in the direction of travel.** Ascending finds the nearest face only for positive delta. For negative delta the nearest face is the *highest* index — resolve against the lowest and the entity stays embedded in the layer beyond it. This only bites when the box straddles two solid layers, which is exactly the already-embedded state a collision fix is supposed to recover from.
...
- **Follow-up, since fixed:** the scans also picked the wrong block when an entity straddled *two* solid layers. `moveAxis` returned on the first solid block found scanning ascending, and `Engine.ts`'s copy did not stop at all, so the last block scanned won. Ascending is right for **positive** delta (travelling `+X`, the nearest face is the *lowest* index) and wrong for **negative** delta (travelling `-X`, you meet the *highest* index first). Resolving `-X` against the low index left the entity still inside the high one, and `resolveOverlap`'s foot-safety then extruded it vertically — an embedded player rode a 2-thick wall up and out of the top. Fixed by `firstBlockingLayer` in `@engine/physics`, which scans the moving axis in the direction of travel; all three copies now use it. Note this also corrected the loop *structure*: the old nesting iterated `bx` outermost regardless of axis, so a Y-axis move never scanned its own axis in order at all.
- **Dead state, since removed:** `hadHorizCollision` on `PlayerController` was written in three places and read nowhere — the vestige of a removed auto-jump system. Found while auditing this bug and deleted in a follow-up commit. It is worth noting how it survived: write-only private state produces no lint warning and no type error, so nothing flagged it for the months between the two changes.
